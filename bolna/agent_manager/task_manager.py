    ...
    async def _process_conversation_task(self, message, sequence, meta_info):
        ...
        if route is not None:
            ...
            if route in self.vector_caches:
                ...
                convert_to_request_log(message=message['data'], meta_info=meta_info, component=LogComponent.LLM, direction=LogDirection.REQUEST, model=self.llm_config["model"], run_id=self.run_id)
                convert_to_request_log(message=message['data'], meta_info=meta_info, component=LogComponent.LLM, direction=LogDirection.RESPONSE, model=self.llm_config["model"], is_cached=True, run_id= self.run_id)
                messages = copy.deepcopy(self.history)
                # TODO revisit this
